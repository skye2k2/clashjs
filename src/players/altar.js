  ...
  ai: function (playerState, enemiesStates, gameEnvironment) {
    ...
    enemiesStates.forEach(function (enemy) {
      if (enemy.isAlive === true) {
        enemy.nearestAmmoDistance = calculateDistanceToNearestAmmo(enemy.position, enemy.direction);
        enemies.push(enemy);
      }
    });
    ...
    params.nearestAmmo = getNearestAmmo(playerState.position, playerState.direction);
    ...
    function calculateVulnerabilityLevel(targetPosition) {
      ...
      if (vulnerabilityLevel === 0.0) {
        ...
        // TODO check for the other two corners?
        enemies.forEach(function (enemy) {
          if (enemy.ammo > 0) {
    ...
    /**
     * True if we are in the same column or row, but NOT the exact same position.
    ...
    function getNearestAmmo(position, direction) {
      var nearestAmmo = null;
      var nearestDistance = null;
      gameEnvironment.ammoPosition.forEach(function (ammo) {
        var distance = getMovesTo(position, direction, ammo);
    ...
    function calculateDistanceToNearestAmmo(position, direction) { // TODO calculate moves not distance
      var nearestDistance = null;
      gameEnvironment.ammoPosition.forEach(function (ammo) {
        var distance = getMovesTo(position, direction, ammo);
        if (nearestDistance === null || distance < nearestDistance) {
